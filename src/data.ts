...
export function applyVenueConfigOverride(config: VenueConfig) {
  const index = venueConfigs.findIndex((item) =>
    item.profile.id === config.profile.id || item.profile.slug === config.profile.slug
  )

  if (index >= 0) {
    venueConfigs[index] = config
    venueProfiles[index] = config.profile
    return
  }

  venueConfigs.push(config)
  venueProfiles.push(config.profile)
}
