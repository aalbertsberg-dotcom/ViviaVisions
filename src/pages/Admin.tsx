...
type AdminProps = {
  ...
  onAddWedding: (input: { couple: string; date: string; guests: number; packageId: string; primaryEmail: string; partnerEmail?: string }) => string | null | Promise<string | null>
...
export default function Admin({ venueId, weddings, activeWeddingId, onSelectWedding, onOpenWedding, onAddWedding, authenticated, authLoading, onAuthenticate, onExitPreview, onLogout, onNavigate, platformAdminAccess = false }: AdminProps) {
  ...
  const submitWedding = async (event: FormEvent) => {
    event.preventDefault()
    const result = await onAddWedding({ couple, date, guests, packageId, primaryEmail, partnerEmail })
    if (result) { setFormError(result); return }
    setShowAdd(false); setCouple(''); setDate(''); setGuests(100); setPackageId(packages[0]?.id ?? ''); setPrimaryEmail(''); setPartnerEmail(''); setFormError('')
  }

  const accessSegment = clientLabel === 'couple' ? 'couple' : 'client'
  const coupleLink = (wedding: WeddingWorkspace) => buildPublicAppUrl(`#/venue/${encodeURIComponent(venue.slug)}/${accessSegment}/${encodeURIComponent(wedding.accessSlug)}`)
  ...
  const copyInvite = (wedding: WeddingWorkspace) => {
    const demoAccess = isDemoClientWorkspace(venue.slug, wedding.accessSlug)
    const text = demoAccess
      ? `${venue.shortName} ${eventLabel} portal: ${coupleLink(wedding)}  Access code: ${wedding.accessCode}`
      : `${venue.shortName} ${eventLabel} portal: ${coupleLink(wedding)}  Sign in or create an account using the email address the venue has on file.`
    return copyText(text, demoAccess ? 'Demo invite' : 'Secure client invite')
  }
  ...
  return (
    <main className="page-main shell admin-page venue-admin" style={{ '--venue-primary': venue.brandPrimary, '--venue-accent': venue.brandAccent, '--venue-surface': venue.brandSurface ?? '#f4f4f4' } as CSSProperties}>
      <section className="page-intro page-intro--split admin-intro">
        ...
        <div className="owner-session-actions"><span className="prototype-badge prototype-badge--large">{venue.isSample ? venue.previewLabel : 'LIVE VENUE'}</span><button className="text-link" onClick={onLogout}>{platformAdminAccess ? `Back to ${PLATFORM_NAME} Admin` : 'Sign out'}</button></div>
      </section>
      ...
      <section className="owner-quick-grid">
        ...
        <button className="owner-quick-card" onClick={() => onNavigate('inventory-admin')}><span>{inventoryTitle.toUpperCase()}</span><strong>Manage inventory records</strong><small>Photos, quantities, storage, package tiers and CSV import</small><b>→</b></button>
        <button className="owner-quick-card" onClick={() => onNavigate('venue-content')}><span>VENUE SETUP</span><strong>Manage venue content</strong><small>Profile, packages and planning spaces</small><b>→</b></button>
      ...
      <section className="panel inventory-admin"><div className="panel__heading"><div><p className="eyebrow">{inventoryTitle.toUpperCase()}</p><h2>Storage &amp; availability</h2><p>{venue.isSample ? 'This inventory is fictional and exists to demonstrate another venue configuration.' : 'The initial Pinrose Prop Shop catalog is configured from public venue information. Working counts, dimensions, storage locations and package access can be finalized during the venue inventory pass.'}</p></div><button className="button button--small button--primary" onClick={() => onNavigate('inventory-admin')}>Manage inventory</button></div><div className="admin-table-wrap"><table className="admin-table admin-table--inventory"><thead><tr><th>Item</th><th>Category</th><th>Tier</th><th>Available</th><th>Storage location</th></tr></thead><tbody>{inventory.map((item) => <tr key={item.id}><td><strong>{item.name}</strong></td><td>{item.category}</td><td>{item.accessTier}</td><td>{item.quantity}</td><td><code>{item.storage}</code></td></tr>)}</tbody></table></div></section>
    </main>
  )
}
